...
namespace  Discrete.ChinesePostman
{
    public class ChinesePostman
    {
        ...
        private static List<(Node, Node)> GetOddNodesCombinations(List<Node> oddNodes)
        {
            ...
            else
            {
                ...
                {
                    // if (ChinesePostman.graph.Edges.Any(x => x.Source == firstNode.Id && x.Destination == node.Id))
                    //     currentPair = (firstNode, node);
                    // else if (ChinesePostman.graph.Edges.Any(x => x.Destination == firstNode.Id && x.Source == node.Id))
                    //     currentPair = (node, firstNode);
                    currentPair = (firstNode, node);
